...
from model_skeleton.BiGAN.dataloader import MNISTDigits, FashionMNIST, EMNIST
from model_skeleton.BiGAN.utility.RBF import trainRBF, RBF
from model_skeleton.BiGAN.base import ConvBlock, ConvTransposeBlock, get_args
import os
...
class Decoder (nn.Module):
    ...
    def __init__(self, X_dim=[1, 28, 28], latent_dim=16):
        ...
        k = 4*latent_dim
        self.rbfNN = RBF(centers=pt.zeros(k, latent_dim),
                         bandwidth=pt.zeros(k), X_dim=np.prod(X_dim))
        ...
        self.lin = nn.Linear(latent_dim, conv2_outchannels *
                             self.conv_outputshape[0]*self.conv_outputshape[1])

        self.conv = nn.Sequential(
            ConvTransposeBlock(conv2_outchannels,
                               conv1_outchannels, kernel_size=3, stride=2),
    ...
    def forward(self, z):
        ...
        """
        x = self.lin(z)
        x = x.view(z.shape[0], -1, self.conv_outputshape[0],
                   self.conv_outputshape[1])
        x = self.conv(x)
        mean = self.Xmean(x)
...
def train(dataloader, latent_dim=2, lr=5e-3, max_epochs=100, device=None):
    ...
    if device is None:
        device = pt.device(
            'cuda') if pt.cuda.is_available() else pt.device('cpu')
    ...
    scheduler = pt.optim.lr_scheduler.MultiplicativeLR(
        optimizer, lambda epoch: 0.95)
    ...
    for epoch in range(max_epochs):
        ...
        for X_input in dataloader:
            with pt.set_grad_enabled(True):
                ...
                kl_loss = -0.5 * (1 + zlogvar - zmean**2 -
                                  zlogvar.exp()).sum(dim=1)
                ...
                for _ in range(L):
                    ...
                    rec_loss += 0.5 * Xlogvar.sum(dim=[1, 2, 3]) + 0.5 * (
                        (X_input - Xmean)**2 / Xlogvar.exp()).sum(dim=[1, 2, 3])
    ...
    rbfNN = trainRBF(modelE, modelD, dataloader, latent_dim, X_dim, k=4*latent_dim,
                     zeta=1e-2, curveMetric=1, max_epochs=50, batch_size=dataloader.batch_size)
...
if __name__ == "__main__":
    ...
    modelE, modelD = train(
        dataloader, latent_dim=args.latent_dim, lr=args.lr, max_epochs=args.epochs)
