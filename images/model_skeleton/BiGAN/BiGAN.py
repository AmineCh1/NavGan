...
from model_skeleton.BiGAN.base import LinBlock, ConvBlock, ConvTransposeBlock, get_args
from model_skeleton.BiGAN.dataloader import MNISTDigits
...
def train(dataloader, latent_dim=2, max_epochs=100, device=None):
    ...
    optimGE = pt.optim.Adam(list(modelE.parameters()) +
                            list(modelG.parameters()), lr=5e-3, weight_decay=1e-4)
    optimD = pt.optim.Adam(modelD.parameters(), lr=3e-3, weight_decay=1e-4)

    schedulerGE = pt.optim.lr_scheduler.MultiplicativeLR(
        optimGE, lambda epoch: 0.95)
    schedulerD = pt.optim.lr_scheduler.MultiplicativeLR(
        optimD, lambda epoch: 0.95)
    ...
    for epoch in range(max_epochs):
        ...
        for X_input, _ in dataloader:
            ...
            with pt.set_grad_enabled(True):
                ...
                lossD = crit(fake_out, fake_scores) + \
                    crit(real_out, real_scores)
                modelD.zero_grad()
                lossD.backward()
                optimD.step()
                ...
                lossGE = crit(fake_out, real_scores) + \
                    crit(real_out, fake_scores)
        ...
        print(
            f"Epoch [{epoch+1}/{max_epochs}]: D Loss {D_loss:.4e}, G Loss {G_loss:.4e}")
...
if __name__ == "__main__":
    ...
    modelE, modelG = train(
        dataloader, latent_dim=args.latent_dim, max_epochs=args.epochs)
